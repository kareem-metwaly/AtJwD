...
class DatasetFromHdf5_JAT(data.Dataset):
    def __init__(self, file_path, patch_size):
        super(DatasetFromHdf5_JAT, self).__init__()
        hf = h5py.File(file_path, 'r')
        self.input = hf.get('INPUT')
        self.t = hf.get('t')
        self.A = hf.get('A')
    ...
    def __getitem__(self, index):
        ...
        t = self.t[index, :, :, :]
        A = self.A[index, :, :, :]
        GT = self.target[index, :, :, :]

        # cv2.imwrite('./train_input.png', self.input[index,:,:,:]*255)
        # cv2.imwrite('./train_output.png', self.target[index, :, :, :]*255)
        return torch.from_numpy(HZ).float(), torch.from_numpy(GT).float(), torch.from_numpy(A).float(), torch.from_numpy(t).float()
