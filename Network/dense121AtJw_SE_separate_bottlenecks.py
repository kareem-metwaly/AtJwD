...
class Model(nn.Module):
    def __init__(self):
        ...
        self.decoder_A = Dense_SEdecoder()
        self.decoder_T = Dense_SEdecoder()
        self.decoder_J = Dense_SEdecoder()
        self.decoder_w = Dense_SEdecoder()
        # self.decoder_B = Dense_decoder()
        # self.convA1 = nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1)
        # self.ResA = ResidualBlock(32)
        # self.convA = nn.Conv2d(32, 3, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, opt):
        ...
        ######################################
        A = self.decoder_A(x, x1, x2, x4AT, opt)
        T = self.decoder_T(x, x1, x2, x4AT, opt)
        T = self.sigT(self.convT(self.ResT(self.convT1(T))))
        T = T.repeat(1, 3, 1, 1)
        J_AT = self.JGen(A=A, t=T, I=x)

        J_direct = self.decoder_J(x, x1, x2, x4J, opt)

        w = self.decoder_w(x, x1, x2, x4W, opt)
